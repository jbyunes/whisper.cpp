...
//JBY
static bool compare_results(const whisper_token_candidate &i1, const whisper_token_candidate &i2) {
  return (i2.p < i1.p);
}
//EOJBY
...
static whisper_token_data whisper_sample_token(
  ...
  if (best) {
    //JBY
    whisper_token_candidate jbyresult = {
      0, 0.0f
    };
    //    float jbyp = .0f;
    //    WHISPER_PRINT_DEBUG("Logits %d\n",n_logits);
    std::vector<whisper_token_candidate> jbyv;
    //EOFJBY
    for (int i = 0; i < n_logits; ++i) {
      // JBY
      jbyresult.id   = i;
      jbyresult.p    = probs[i];
      //      jbyp += jbyresult.p;
      if (probs[i]>1e-6f) jbyv.push_back(jbyresult);
      // EOFJBY
      if (result.p < probs[i]) {
        result.id   = i;
        result.p    = probs[i];
    ...
    //JBY
    std::sort(jbyv.begin(), jbyv.end(), compare_results);
    //    WHISPER_PRINT_DEBUG("total proba=%f\n",jbyp);
    result.n_candidates = MAX_CANDIDATES;
    if (jbyv.size()<MAX_CANDIDATES) result.n_candidates = jbyv.size();
    for (int i=0; i<result.n_candidates; i++) {
      result.candidates[i] = jbyv[i];
    }
    int jbyi = 0;
    for (whisper_token_candidate d: jbyv) {
      WHISPER_PRINT_DEBUG("%s %d id=%d p=%f %s\n",__func__,jbyi,d.id,d.p,whisper_token_from_id(&ctx,d.id));
      if (jbyi++==MAX_CANDIDATES) break;
    }
    //EOJBY
  } else {
    //JBY
    WHISPER_PRINT_DEBUG("%s not best\n",__func__);
    //EOJBY
...
int whisper_full_with_state(
  ...
  while (true) {
    if (params.progress_callback) {
      ...
      params.progress_callback(ctx, ctx->state, progress_cur, params.progress_callback_user_data);
    }
    ...
    for (int it = 0; it < (int) temperatures.size(); ++it) {
      ...
      //JBY
      //      WHISPER_PRINT_DEBUG("nmax=%d\n", whisper_n_text_ctx(ctx)/2 - 4);
      //EOJBY
      for (int i = 0, n_max = whisper_n_text_ctx(ctx)/2 - 4; i < n_max; ++i) {
        ...
        //JBY
        //        WHISPER_PRINT_DEBUG("n_decoders_cur=%d\n",n_decoders_cur);
        //EOJBY
        for (int j = 0; j < n_decoders_cur; ++j) {
          //JBY
          //          WHISPER_PRINT_DEBUG("decoder %d\n",j);
          //EOJBY
          auto & decoder = state->decoders[j];
        ...
        if (params.strategy == whisper_sampling_strategy::WHISPER_SAMPLING_BEAM_SEARCH) {
          ...
          for (int j = 0; j < n_decoders_cur; ++j) {
            //JBY
            WHISPER_PRINT_DEBUG("loop j=%d ndecoders_cur=%d\n",j,n_decoders_cur);
            //EOJBY
            auto & decoder = state->decoders[j];
    ...
    // output results through a user-provided callback
    {
      ...
      for (int i = 0; i < result_len; ++i) {
        //JBY
        auto t = tokens_cur[i];
        WHISPER_PRINT_DEBUG("%s: i = %d result_len = %d token_cur.id %d t0 %lld t1 %lld\n",__func__,i,result_len,t.id, t.t0, t.t1);
        //EOJBY
        ...
        for (int i = 0; i < (int) tokens_cur.size(); i++) {
          ...
          if (tokens_cur[i].id > whisper_token_beg(ctx) && !params.single_segment) {
            ...
            if (!text.empty()) {
              ...
              //JBY remplace le commentaire qui suit...!
              result_all.push_back({ tt0, tt1, text, {}, speaker_turn_next, best_decoder.sequence.avg_logprobs , best_decoder.sequence.entropy, best_decoder.sequence.score });
              //CORRECT!!!              result_all.push_back({ tt0, tt1, text, {}, speaker_turn_next });
              //EOJBY
              for (int j = i0; j <= i; j++) {
...
//JBY
double whisper_full_get_segment_entropy(struct whisper_context * ctx, int i_segment) {
  return ctx->state->result_all[i_segment].entropy;
}

double whisper_full_get_segment_avg_logprobs(struct whisper_context * ctx, int i_segment) {
  return ctx->state->result_all[i_segment].avg_logprobs;
}

double whisper_full_get_segment_score(struct whisper_context * ctx, int i_segment) {
  return ctx->state->result_all[i_segment].score;
}
//EOJBY
